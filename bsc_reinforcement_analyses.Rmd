...
First we need to determine interactions. Using GPS tracking data we cannot perfectly capture social interactions, but we can approximate these with proximity (distance and time) analysis. The dynamic interaction (DI) index takes into account cohesiveness of movement, giving a statistic between zero (little cohesiveness) and one (cohesivenss). However DI is irrespective of distance, so proximity should first be considered.
...
We calculate the Prox and DI statistics for all pairs for the two time periods of interest:
...
And matrix of potential interactions (overlap periods)
...
Calculate association indices i.e., contacts divided by total number of potential interactions.
...
Calculate social network stats using igraph.
...
# plot communities with weighted edges and vary node size by node strength
plot(res_graph, vertex.color=membership(comm),
     edge.width = E(res_graph)$weight/1000,
     vertex.label = NA,
...
Plot using ggraph for more versatile aesthetic options.

```{r}
# convert from igraph to tidygraph
res_tidy <- as_tbl_graph(res_graph) %>%
  mutate(centrality = as.numeric(centrality_degree(mode = 'in')),
         membership = as.factor(group_walktrap(weights = weight)))
...
Seeing what happens if I scale the weights and remove very small values since the range is 1-5000 for contacts
...
plot(rein_graph,
     ...
     edge.width = E(rein_graph)$weight/200)
```
...
Reinforcers moved significantly further per night than residents (estimate = 4.287, SE = 1.940, t = 2.210, P = 0.033). Both cohorts travelled further with additional time post-release (estimate = 0.223, SE = 0.021, t = 10.818, P < 0.001) but the rate of increase was steeper for residents (estimate = 0.068, SE = 0.027, t = -2.531, P = 0.012).
...
We found a significant effects of cohort, elapsed time, and the interaction of these on the daily distance between roosts. Both cohorts decreased their movement over time, but residents decreased more steeply than reinforcers.

Reinforcers moved significantly further between consecutive roosts than residents (estimate = 0.412, SE = 0.157, t = 2.620, P = 0.010). Both cohorts decreased the distance between roosts over time (estimate = -0.023, SE = 0.003, t = -7.229, P < 0.001) but the rate of decline was steeper for residents (estimate = -0.011, SE = 0.004, t = 2.665, P = 0.008). Additionally, reinforcers settled in more quickly than residents; the initial spike and decline in movement between roosts took four days to stabilise in the reinforcing cohort and eleven days in the resident cohort. 
...
We found a significant effects of cohort, elapsed time, and the interaction of these on the distance from release. Residents exhibit higher initial values for dist_release, but their rate of increase over time is slower than that of the baseline cohort. There is a significant difference between cohorts at time zero.

Reinforcers moved significantly shorter distances from the release site compared to residents (estimate = -5.306, SE = 2.310, t = -2.297, P = 0.028). Both cohorts moved further away from the release site increased with additional time post-release (estimate = 0.023, SE = 0.010, t = 2.425, P = 0.015) but the rate of increase was steeper for the reinforcers (estimate = 0.084, SE = 0.013, t = 6.656, P < 0.001). There was a significant difference between cohorts at time zero (p = 0.0282), where residents immediately moved further from the release site (average 221m further than reinforcers).
...
We found a significant effects of cohort, elapsed time, and the interaction of these on home range area. Reinforcers initially have much large home ranges (due to the shuttle between the central woodland and the eastern fence line) than the residents, before picking a core roost.Reinforcers decline over time while residents increase. 
...
Reinforcers had significantly larger home ranges than residents (estimate = 1.926, SE = 0.190, t = 10.143, P < 0.001). Home range area increased significantly with time (estimate = 0.0224, SE = 0.0024, t = 9.372, P < 0.001). However, this relationship was moderated by cohort status: while residents showed a continued increase in area over time, reinforcers exhibited a slight decline in area over time (estimate = -0.0220, SE = 0.0031, t = -7.102, P < 0.001), suggesting a divergence in the trajectories between cohorts.
...
Significant effects of cohort, time and their interaction. The effect of elapsed time depends on the cohort, with residents increasing more over time than reinforcers.

Reinforcers allocated significantly more time to the foraging state than residents (estimate = 0.372, SE = 0.098, z = 3.791, P < 0.001). 
...
Plot all these results together for ease of viewing.
...
The probability of survival to 54 days for residents is 88% (upper CI = 100%, lower CI = 71%). 

The probability of survival to 54 days for reinforcers is 95% (upper CI = 100%, lower CI = 86%). 
