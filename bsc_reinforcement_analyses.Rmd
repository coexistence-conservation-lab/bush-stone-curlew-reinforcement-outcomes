...
# Packages
pacman::p_load(atlastools, beepr, janitor, ggmap, tidyverse)
...
We first tested the pipeline with a subset of the data (bird "Marmalade").
...
```{r}
# Histogram altitude
hist(data_dup$height_above_msl)

# Summary statistics altitude
summary(data_dup$height_above_msl)
quantile(data_dup$height_above_msl, probs = 0.99)
...
The median flight height was 129m, the minimum was -1974m and the maximum was 9999m. These min and maximums were considered unrealistic.

Most of the data that looks real is at ground altitude. Many of the obvious spikes have high or low altitude. 

We cut off the maximum altitude at 262m (at the 0.99 quartile) and the minimum to 20m (ground elevation asl at Mt Rothwell). 
...
We removed fixes taken with less than 4 satellites using the dupfilter from SDLfilter. 
...
# Remove duplicate values and satellite count <4
data_dup <- dupfilter(data_raw)
```

dupfilter_exact removed 2 of 1776702 locations
dupfilter_qi removed 3660 of 1776700 locations
dupfilter_time removed 1 of 1773040 locations
dupfilter_space removed 1896 of 1773039 locations

Input data: 1776702 locations
Filtered data: 1771143 locations
dupfilter removed 5559 locations (0.31% of original data)

## Altitude filtering

Incorrect GPS fixes often have incorrect altitude (due to the coincidence of horizontal and vertical dilution of precision).High altitude was not expected as no birds were undertaking long-distance flight, which is the only time high altitude is possible. All birds were wing-clipped on release and some later moulted and undertook short-distance flight, but not at high altitude. 
...
```{r}
# Filter data to realistic altitude
data_alt <- data_dup %>%
  filter(height_above_msl < 262 & height_above_msl > 20)

# Histogram altitude
hist(data_alt$height_above_msl)
```

Input data: 1771143 locations
Filtered data: 1719843 locations
Altitude filtering removed 51300 locations (2.90% of original data)
...
```{r Gundog GPS filter, include=FALSE}
#############################################Gundog.GPS.Filter#############################################
...
A test case on one bird was previously used to refine the user input threshold values until visual inspection showed good performance on reducing obvious outliers (not included here). This was applied to all birds in this study. 

Note: takes 5-30 seconds per bird.
...
We then plotted the filtering results against a satellite map to visualise and check the effectiveness. 
...
We save the filtered data to a smaller file (less columns). 
...
Input data: 1719843 locations
Filtered data: 1701926 locations
Gundog filter removed 17917 locations (1.04% of original data)