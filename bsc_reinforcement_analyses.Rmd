...
All comparisons made for the first 55 days post-release, since this is the lowest common denominator of duration tracked for the reinforcing cohort.
...
datam1 <- read.csv("data/daily_distance_moved.csv") %>%
  # add metadata
...
We found a significant effects of cohort, elapsed time, and the interaction of these on the daily distance moved. Residents changed their movement more steeply over time than reinforcers.
...
```{r}
# read in data and filter to establishment period
datam2 <- read.csv("data/daily_distance_between_roosts.csv") %>%
  # add metadata
  left_join(dplyr::select(metadata, c("id", "cohort", "start_date"))) %>%
  # format dates as dates
  mutate(date = as_date(date),
...
```{r}
# read in data and filter to establishment period
datam3 <- read.csv("data/daily_roost_distance_from_release.csv") %>%
  # add metadata
  left_join(dplyr::select(metadata, c("id", "cohort", "start_date"))) %>%
  # format dates as dates
  mutate(date = as_date(date),
...
```{r}
# read in data and filter to establishment period
datam4 <- read.csv("data/hr_daily_area.csv") %>%
  # add metadata
  left_join(dplyr::select(metadata, c("id", "cohort", "start_date"))) %>%
  # format dates as dates
  mutate(date = as_date(date),
...
We hypothesised members of the mixed resident-reinforcer social group would proceed more quickly through the PRBM phases than those in the reinforcer only group. 
...
```{r}
# read in data and filter to reinforcers during study period
dist <- read.csv("data/daily_distance_moved.csv") %>%
  # add metadata
  left_join(dplyr::select(metadata, c("id", "cohort", "start_date"))) %>%
  # format dates as dates
  mutate(date_bird = as_date(date_bird),
...
# plot daily distance over time by social group
ggplot(dist)+
...
Visually, the mixed group seems to move further later, but test with stats.

```{r}
# distribution of movement data
hist(dist$daily_dist)
ggqqplot(dist$daily_dist)

# test whether establishment differs between the groups
m5a <- lmer(daily_dist ~ social * elapsed + (1|id), data = dist)
summary(m5a)
anova(m5a)
```

Significant effect of social group and time, but not social group alone. 

What about if we compare the social groups to the residents?
...
dist <- read.csv("data/daily_distance_moved.csv") %>%
  ...
  filter(date_bird %in% rein_dates) %>%
  # add column for membership to social group
  mutate(social = ifelse(id %in% club, "exclusive", ifelse(cohort == "Resident", "mixed-resident", "mixed-reinforcer"))) 

# plot daily distance over time by social group
ggplot(dist)+
  geom_smooth(aes(date_bird, daily_dist, color = social), method = "lm")+
  theme_minimal() +
  ylab("Daily distance moved (m)")+
  theme(axis.title.x=element_blank())

# test whether distance moved differs between the groups
m5b <- lmer(daily_dist ~ social + (1|id), data = dist)
summary(m5b)
anova(m5b)
TukeyHSD(aov(daily_dist ~ social, data = dist))
```

The residents and exclusive reinforcer groups are significantly different, but the other pairwise comparisons do not differ. 

### m7: Roost establishment

```{r}
# read in data and filter to reinforcers during study period
roost <- read.csv("data/daily_distance_between_roosts.csv") %>%
  # add metadata
  left_join(dplyr::select(metadata, c("id", "cohort", "start_date"))) %>%
  # format dates as dates
  mutate(date = as_date(date),
...
# calculate social group means
summary <- roost %>%
  group_by(social, elapsed) %>%
  summarise(mean_dist = mean(distance_previous))

# plot distance moved by cohort during establishment
ggplot(summary)+
  geom_path(aes(elapsed, mean_dist, colour = social))+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("Days post-release") + 
  ylab("Mean daily distance between roosts (m)")

# plot again but trend instead of path
ggplot(roost)+
...
The mixed group establishes more fixed roost locations, moving less over time. 

Test with stats. 

```{r}
# distribution of movement data
hist(roost$distance_previous)
ggqqplot(roost$distance_previous)

# correct left skew
hist(log(roost$distance_previous))
ggqqplot(log(roost$distance_previous))

# test whether establishment differs between the groups
m6 <- lmer(log(distance_previous) ~ social * elapsed + (1|id), data = roost)
summary(m6)
anova(m6)
```

The social groups are significantly different (but no effect of time).
...
# test whether distance moved differs between the groups
m6b <- lmer(distance_previous ~ social * scale(date) + (1|id), data = roost)
summary(m6b)
anova(m6b)
TukeyHSD(aov(distance_previous ~ social, data = roost))
```
