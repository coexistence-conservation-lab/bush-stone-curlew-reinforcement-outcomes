...
How long does it take birds to settle? Do acclimation periods differ between cohort.
...
First calculate roost locations. Test with one bird, Aurora. 
...
# locate kmeans centers per bird per day
...
for (i in 1:length(birds)){
  for (j in 1:length(days)){
    ...
    # find kmean centers
    kmean <- kmeans(subset, centers=2) # 2 centers to allow for secondary roost site
    
    # calculate distance between centers
    coords <- kmean[["centers"]] %>%
    vect(crs = "EPSG:32755") %>%
    st_as_sf()
      
    dist <- as.numeric(st_distance(coords[1,], coords[2,]))
      
    
    # compile data
    out <- as.data.frame(cbind(kmean$centers, npoints = kmean$size)) %>%
      mutate(id = birds[i], date = days[j], dist = dist)
    
    # remove the smaller of a pair of roosts, only when distance between is < 30
    
    if(max(out$dist)<=30) {
      out <- out %>% 
        filter(npoints == max(npoints))
    }
    
    clusters <- rbind(clusters, out)
...
hist(clusters$npoints)
summary(clusters$npoints)

# plot clusters by number of points
ggplot()+
  geom_point(data = clusters, aes(easting, northing, size = npoints, colour = id))+
  geom_path(data = clusters, aes(easting, northing, group = id), alpha = 0.3)+
  coord_sf()+
  scale_colour_viridis_d()+
  theme_bw()

# remove clusters with less than 15 points, insufficient to constitute a roost
roosts <- clusters %>%
  filter(npoints > 15)

# plot roosts by number of points
ggplot()+
  geom_point(data = roosts, aes(easting, northing, size = npoints, colour = id))+
  geom_path(data = roosts, aes(easting, northing, group = id), alpha = 0.3)+
  coord_sf()+
  scale_colour_viridis_d()+
  theme_bw()

# plot for a single bird with date as colour
ggplot(data = filter(roosts, id == "Aurora"))+
...
distance_roosts_latlon <- distance_roosts %>%
  ...
  project("EPSG:4326") %>%
  st_as_sf() %>%
  # drop the very far nutmeg outlier roost and return
...
# and same plot on map
ggmap(map_z15)+
  geom_sf(data = distance_roosts_latlon, aes(colour = distance_previous), inherit.aes = FALSE)+
  scale_colour_viridis_c()

# write to file
write.csv(distance_roosts_latlon, "data/distance_between_daily_roosts")
```
...
dist <- roosts %>%
  ...
  # arrange by date
  arrange(date) %>%
  # remove the extreme value for nutmeg, visual error
  filter(dist_release < 2000)

# plot distance over time
ggplot(dist)+
  geom_path(aes(date, dist_release, colour = id))+
  scale_colour_viridis_d()+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("Days post-release") + 
...
# convert to latlon and back to data frame
dist_latlon <- dist %>%
  vect() %>%
  project("EPSG:4326") %>%
  st_as_sf() %>%
  st_coordinates() %>%
  as.data.frame() %>%
  cbind(st_drop_geometry(dist), .) %>%
  clean_names() %>%
  arrange(date, id)

# plot roost location over time
ggmap(map_z15)+
  geom_point(data = dist_latlon, aes(x, y, colour = date), alpha = 0.7)+
  scale_colour_viridis_c()+
  theme_void()
```
