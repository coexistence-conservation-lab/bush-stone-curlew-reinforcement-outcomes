...
# Packages
pacman::p_load(adehabitatHR, amt, asnipe, atlastools, beepr, emmeans, ggfortify, ggmap, ggnewscale, ggforce, ggpubr, ggraph, ggridges, glmmTMB, gtools, igraph, janitor, jtools, lme4, lmerTest, move2, momentuHMM, readxl, scales, scattermore, sf, sp, STRAND, suncalc, survival, survminer, terra, tidygraph, tidyterra, tidyverse, viridis, wildlifeDI)
...
# make symmetrical
prox[lower.tri(prox)] <- t(prox)[lower.tri(prox)]

# convert to graph object
network <- graph_from_adjacency_matrix(prox, mode = "undirected", diag = FALSE, weighted = TRUE)
...
distances(network, algorithm="unweighted")

# proportion of node's neighbors that are connected to each other
summary(transitivity(network, "local"))
```
...
w <- cluster_walktrap(network)
length(w)
modularity(w)
membership(w)

# community with spinglass
sg <- cluster_spinglass(network)
length(sg)
modularity(sg)
membership(sg)

# community with infomap
im <- cluster_infomap(network)
length(im)
modularity(im)
membership(im)
```
...
ggraph(comm)+
  ...
  scale_fill_manual(values = c("skyblue", "thistle2"))+
  # add labels
  #geom_node_text(aes(label = name), nudge_y = 0.17)+
  # theme
  theme_void()+
  theme(legend.position="none")+
...
```{r}
# read in dyad proximity scores and convert to association matrix
prox <- read.csv("results/proximity_reinforcers.csv") %>%
  # add rows for last and first bird self comparison
...
Calculate community with walktrap method and plot the network.
...
Instead of using dyad connections, we can use the social groups (which are serendipitously nearly evenly split among the reinforcers) to ask questions about conspecific interactions. Social groups can tell you more about the overal social environment than dyads alone (Farine and Whitehal 2015).
...
The mean distance moved (and distance moved over time) for the mixed-reinforcers was more similar to the residents than the exclusive-reinforcer group. All groups increase over time. 
...
m1 <- lmer(daily_dist ~ social * scale(date) + (1|id), data = datam1)
summary(m1)
anova(m1)

emmeans(m1, pairwise ~ social | date, adjust = "Tukey")
...
All founders moved greater distances per night with increasing time post release (estimate = 115.23, SE = 37.06, p= 0.0019) but this increase was steeper for residents (p = 0.0035) and mixed reinforcers (p = 0.0144) than exclusive reinforcers. 

Increasing movement over time may indicate more effective exploitation of resources and/or larger area of exploitation.
...
Exclusive reinforcers travelled significantly greater distances between consecutive roosts than both mixed reinforcers (p = 0.0010) and residents (p < 0.0001). The rate of change in distance moved over time was not significantly different between groups (p > 0.668).

The larger distances between roosts indicates the exclusive group were less settled and kept moving their roost location rather than establishing a static home range.
...
ggplot(sum3)+
  geom_path(aes(date, dist_release, color = social), linewidth = 1)+
  theme_minimal() +
  xlab("Date") + 
  ylab("Distance from release site (m)")+
  scale_colour_manual(values = c("#440154", "#F08080", "#FFD700"))
...
ggplot(datam3)+
  geom_smooth(aes(elapsed, dist_release, color = social), method = "lm")+
  theme_minimal() +
  xlab("Days post-release") + 
  ylab("Distance from release site (m)")+
  scale_colour_manual(values = c("#440154", "#F08080", "#FFD700"))
...
m3 <- lmer(sqrt(dist_release) ~ social * scale(date) + (1|id), data = datam3)
summary(m3)
...
Exclusive reinforcers moved significantly further from the release site than both mixed reinforcers (p = 0.025) and residents (p = 0.0011). All founders moved away from the release site over time (estimate = 3.3473, SE = 0.1552, p < 0.0001); however, compared to exclusive reinforcers, the residents and mixed reinforcers had significantly slower increases in distance from the release site over time (p < 0.0001). 

Greater release site fidelity in the mixed group, while the exclusive group continued to move away.
...
Multi-plot of all PRBM metrics over time.
...
g1 <- ggplot(datam1)+
  ...
  xlab(element_blank()) + 
  ylab("Daily distance moved (m)")+
  scale_colour_manual(values = c("#440154", "tomato", "#FFD700"),
...
g6 <- ggplot(datam6)+
  ...
  xlab(element_blank()) + 
  ylab("Weight (g)") + 
  scale_colour_manual(values = c("#440154", "#F08080"))
...
Do they remain more attached to residents?
