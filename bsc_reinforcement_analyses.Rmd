...
Here we removed points after the end of the study period (5/8/23, at the start of next experiment - translocation to Orana).
...
We also excluded Star and Wobbles from the analyses because they died early (8 and 9 days).

To save on plotting power, I'm checking with 4 birds: Marmalade and Nutmeg (who had the best flight during the study and left the sanctuary) and Zeus and Riley (who did not have flight but did have very 'jittery' tracks with lots of obvious spikes).
...
# Plot to check
subset_raw <- filter(data_raw, id %in% c("Marmalade", "Nutmeg", "Riley", "Zeus"))
subset_lfilt <- filter(data_lfilt, id %in% c("Marmalade", "Nutmeg", "Riley", "Zeus"))

ggmap(map_z13)+
  geom_path(data = subset_raw, aes(location_long, location_lat), colour = "white")+
  geom_point(data = subset_raw, aes(location_long, location_lat), colour = "red")+
  geom_point(data = subset_lfilt, aes(location_long, location_lat), colour = "blue")+
  scale_colour_viridis_d()+
  theme_void()+
  facet_wrap(~id)

ggsave("plots/filtering step 1.jpeg", dpi = 200, width = 20, height = 20, units = "cm")
```

Percentage original data removed: 1%
...
There are a handful of massive hdop values (5-15) but most are < 2
...
I also filter for duplicate timestamps. 
...
dupes <- data_lfilt %>% 
  ...
  summarise(total_fixes = n(),
            unique_times = n_distinct(datetime),
            duplicates = total_fixes - unique_times)
...
# Plot to check
subset_qfilt <- filter(data_qfilt, id %in% c("Marmalade", "Nutmeg", "Riley", "Zeus"))

ggmap(map_z14)+
  geom_path(data = subset_lfilt, aes(location_long, location_lat), colour = "white")+
  geom_point(data = subset_lfilt, aes(location_long, location_lat), colour = "red")+
  geom_point(data = subset_qfilt, aes(location_long, location_lat), colour = "blue")+
  scale_colour_viridis_d()+
  theme_void()+
  facet_wrap(~id)

ggsave("plots/filtering step 2.jpeg", dpi = 200, width = 20, height = 20, units = "cm")
```
...
To remove spikes in the data we can filter out positions with extreme incoming and outgoing speeds, especially when the birds are turning (where fast speeds are less likely). First we need to define biologically realistic incoming and outgoing speeds.
...
summary(turn$speed_in, na.rm = TRUE)
summary(turn$speed_out, na.rm = TRUE)

# Plot points with angles and speeds - test one bird
temp <- filter(data_speed, id == "Nutmeg")
ggmap(map_z15)+
  geom_path(data = temp, aes(location_long, location_lat), colour = "white")+
  geom_point(data = temp, aes(location_long, location_lat, colour = angle))+
  scale_colour_viridis_c()+
  theme_void() 
ggmap(map_z14)+
  geom_path(data = temp, aes(location_long, location_lat), colour = "white")+
  geom_point(data = temp, aes(location_long, location_lat, colour = speed_in))+
  scale_colour_viridis_c()+
  theme_void()
```

We defined the biologically realistic maximum speed of 1.6 m/s (5.7 km/h from 99th percentile).

Note: would not use this threshold for birds undertaking long-distance movements, during which they can get much greater speeds (>60km/h or 16m/s) especially when gliding with tail winds. 

We defined the biologically realistic turning speed as 0.5m/s for angles greater than 90.

```{r}
# Apply turning angle and speed filtering
data_sfilt <- data_speed %>%
  filter(speed_in < 1.6) %>%
  filter(speed_out < 1.6) %>%
  filter(!(speed_out > 0.5 & angle > 90)) %>%
  filter(!(speed_in > 0.5 & angle  > 90))

# Filtering effect
print(paste("Percentage original data removed: ", round(((nrow(data_qfilt)-nrow(data_sfilt)) / nrow(data_qfilt))*100), "%", sep = ""))

# Plot filtered points by speeds - one bird
temp <- filter(data_sfilt, id == "Zeus")
ggmap(map_z15)+
  geom_path(data = temp, aes(location_long, location_lat), colour = "white")+
  geom_point(data = temp, aes(location_long, location_lat, colour = speed_in))+
  scale_colour_viridis_c()+
...
# Plot to check
subset_sfilt <- filter(data_sfilt, id %in% c("Marmalade", "Nutmeg", "Riley", "Zeus"))

ggmap(map_z14)+
  geom_path(data = subset_qfilt, aes(location_long, location_lat), colour = "white")+
  geom_point(data = subset_qfilt, aes(location_long, location_lat), colour = "red")+
  geom_point(data = subset_sfilt, aes(location_long, location_lat), colour = "blue")+
  scale_colour_viridis_d()+
  theme_void()+
  facet_wrap(~id)

ggsave("plots/filtering step 3.jpeg", dpi = 200, width = 20, height = 20, units = "cm")
...
ggsave("plots/filtering step 3 zoomed.jpeg", dpi = 200, width = 20, height = 20, units = "cm")
```

Percentage original data removed: 10%
...
ggsave("plots/smoothed by id.jpeg", dpi = 300, width = 30, height = 30, units = "cm")
```

## 5) Data thinning

Resampling the tracks to a uniform resolution to be equal among the birds (accounting for variation in device success) and to reduce computation intensity for following stages.

What is the current sampling rate per bird?
...
The highest sampling rates are 11 minutes for Timber and Marmalade, the overall average 6 minutes. Subsampling to 10 minutes with a tolerance of 2 minutes.

```{r}
data_thin <- steps %>%
  mutate(resampled = map(data, ~ track_resample(.x, rate = minutes(10), tolerance = minutes(2)))) %>%
  select(id, resampled) %>%
  unnest(resampled)
```

Originally I kept the utm easting and northing columns from movebank but I noticed a mismatch between the cleaned lat/lon and the utm, so have elected to transform the data instead. Plus they were dropped in the amt steps.

```{r}
# Convert data to spatial and reproject in utm
points <- data_thin %>%
  vect(geom = c("x_", "y_"), crs = "EPSG:4326") %>%
...
data <- data_thin %>%
  ...
  rename(longitude = x_,
         ...
         time_local = t_) %>%
  # add utm xy columns 
  mutate(x = st_coordinates(points$geometry)[,1],
         y = st_coordinates(points$geometry)[,2])
...
ggsave("plots/smoothed thinned by id.jpeg", dpi = 300, width = 30, height = 30, units = "cm")
...
Birds were tracked for a total of 286 days between October 2022 and August 2023 for a total of 3567 tracking days (sum of each bird's tracking duration). 
...
ggplot(data_sun)+
  geom_scattermore(aes(x, y, colour = tod), alpha = 0.6)+
  coord_sf()+
  theme_void()
...
Distance moved is a metric for establishment, where we predict a short-term increase in distance moved immediately post-release as animals become familiar with their new environment, followed by a decrease as they learn to use the resources. This follows the theoretical predictions of exploration and exploitation hypothesis. 

Distance moved is also a proxy for effort, where travelling further to find resources (e.g. food, water) is more costly than travelling less far. Birds with better foraging skills may not have to travel as far to acquire as many resources. Likewise, birds with a good choice of diurnal roost close to nocturnal resources do not have to commute as far. 
...
for(i in 1:length(birds)){
  ...
  steps <- points %>%
    ...
    # format as amt track 
    make_track(.x = x, .y = y, .t = time_local, id = id, 
...
ggsave("plots/distance by id.jpeg", dpi = 200, width = 10, height = 15, units = "cm")
```

## Roost establishment

The establishment of diurnal roosts is an important step for translocated bush stone-curlews to develop wild skills post-release. The distance between consecutive roosts is a good proxy for their establishment, where decreasing distance between roosts indicates they are roosting in a similar area and therefore establishing a pattern of movement- finding and then exploiting suitable resources. 

Calculate diurnal roost locations.
...
days <- unique(data_day$date_bird)
...
ggplot(roosts)+
  geom_point(aes(x, y, size = npoints, colour = id))+
  geom_path(aes(x, y, group = id), alpha = 0.3)+
  coord_sf()+
  scale_colour_viridis_d()+
  theme_bw()
...
ggplot(data = filter(roosts, id == "Marmalade"))+
  geom_point(aes(x, y, size = npoints, colour = as.Date(date)))+
  geom_path(aes(x, y, group = id), alpha = 0.3)+
  coord_sf()+
  scale_colour_viridis_c()+
  theme_bw()+
...
roosts <- roosts %>%
  vect(geom = c("x", "y"), crs = "EPSG:32755") %>%
  st_as_sf()
...
One real roost outside (Nutmeg's adventure as above) and a handful where birds were just roosting close to the fence. This is fine. 

Save output.

```{r}
# convert geometry to regular columns to save to disk
distance_roosts2 <- distance_roosts %>%
  st_drop_geometry() %>%
...
How far did they move from the release location? Based on daily roost distance from release location. Release site fidelity is a metric for translocation success. 
...
To determine the temporal interaction threshold we use half the mean sampling rate.
...
The mean sampling rate for the cleaned data is 11 minutes. We will use a time parameter of 5.5 minutes (half the sampling interval) following advice from the wildlifeDI package vignette. 
...
Our practitioner guesstimate was a threshold of 20m (based on visual observation of interactions between individuals in the field during monitoring). 
...
dcPlot(move_subset,tc=5.5*60,dmax=100)
```

The majority of contacts took place <40m apart. I'll retain the guesstimate of 30m as the parameter. This is also within tolerance for the approximate GPS uncertainty.
...
checkTO(dyad)
Sys.time()
test <- Prox(dyad, tc=5.5*60, dc=30);Sys.time()
```

The test dyad took 34 seconds so I'm anticipating the full set of 78 interactions will take 45 minutes.
...
Mean proximity score is 13%, min is no interaction and max is 69%. 

### Residents and reinforcers

We define this as the period from the last reinforcer released (2023-06-11) to the end of the study period (when some of the reinforcers were moved to Orana on 2023-08-05).
...
# Test for a single dyad
dyad <- filter(data_reinforce, id %in% c("Athena", "Zeus"))
...
The test dyad took 11 seconds so I'm anticipating the full set of 378 interactions will take 1 hour to run. 
...
Mean proximity score is 10.4%, min is <1% and max is 33%. 
...
We have a continuous network of 13 individuals (nodes) and 69 edges with an edge density of 87.2%.
...
Next calculate community. Based on the suggestions by [Yang et al. (2016)](https://www.nature.com/articles/srep30750).
...
# convert to graph object
g2 <- graph_from_adjacency_matrix(prox2, mode = "undirected", diag = FALSE, weighted = TRUE)
...
We have a continuous network of 28 individuals (nodes) and 378 edges with an edge density of 100% (no dyads lack a connection). 

```{r}
# set seed to keep layout static
set.seed(1111)
layout2 <- create_layout(g2, layout = "fr") 

# add cohort membership to layout
layout2 <- layout2 %>%
  rename(id = name) %>%
...
```{r}
# community with spinglass - weights NULL uses the default weights of the dataset
com2 <- cluster_spinglass(g2, weights = NULL)
length(com2)
modularity(com2)
membership(com2)
...
# add community to network layout
layout2 <- layout2 %>%
  mutate(community = as.factor(membership(com2)))
...
Interesting! The reinforcers split nearly perfectly in half, with nine in an exclusive social group (consisting only of reinforcers) and ten in a mixed social group (along with all the residents). Daisy was not a member of either group.
...
n1 <- ggraph(layout1)+
  ...
  expand_limits(x = c(-7.2, -0.1), y = c(-1.1, 2.4))

# reinforcer network
n2 <- ggraph(layout2)+
  # community polygon
  geom_mark_hull(aes(x = x, y = y, fill = community), 
                 colour = "white", alpha = 0.6, expand = unit(0.5, "cm"),
                 show.legend = FALSE)+
  scale_fill_manual(values = c("seagreen3", "#2E3FFF", "grey"))+
  # vary edge alpha by weight
  geom_edge_fan(aes(alpha = weight), show.legend = FALSE) +
  # adjust weight alpha to make visually easier to discern
  ...
  theme_void()+
  theme_graph(foreground = "black", border = TRUE,
              plot_margin = margin(10, 20, 5, 20))+
  expand_limits(x = c(-5.5, -0.2), y = c(1.1, 5.4))
...
We hypothesised members of the mixed resident-reinforcer social group would proceed more quickly through the PRBM phases than those in the reinforcer only group. I also want to compare both reinforcer groups to the residents to see if one is more similar than the other. Excluding Daisy who was not a member of either group.

Dates are from the last reinforcer released to the 3rd August 2023, because on the 4th we commenced fieldwork for the translocation to Orana and so the birds were disturbed by the intervention. 
...
# define reinforcer date period
rein_dates <- seq.Date(from = as_date("2023-06-11"), to = as_date("2023-08-03"), by = "day")
```
...
**Results:**
Distance moved per day increased with time post-release. Both reinforcer groups had significant negative linear and quadratic responses over time compared to residents, meaning the increase of distance over time decelerated and declined for these groups

The exclusive reinforcer group had a significantly lower slope of movement increase over time compared to the residents and mixed reinforcers.

**Interpretation:**
Reinforcers increase over time in response to the need to explore (find resources etc.) then stabilising - evidence of establishment. Also increase in movement may align with theoretical increase in fitness and physical capability with increased time out of captivity. Mixed reinforcers were more similar to the residents than the exclusive reinforcers.
...
emtrends(best2, pairwise ~ social)
...
**Results:**
Roost displacement generally decreased over time. However, this effect was mediated by social group, where exclusive reinforcers moved significantly further than residents and had a significant quadratic interaction such that they initially increased before reaching a peak and declining. The mixed reinforcers were only weakly different from residents. 
...
Distance from the release site significantly increased over time and had a positive quadratic effect, indicating an acceleration of this increase.

Exclusive reinforcers moved significantly further from the release site than both mixed reinforcers and residents (p < 0.0001).
...
Does home range size differ between the social groups? Using 50% KUD because 90% were skewed by the bounds of the fence.
...
# distribution of movement data
hist(datam4$kud50)
ggqqplot(datam4$kud50)

# correct left skew
hist(log1p(datam4$kud50))
ggqqplot(log1p(datam4$kud50))
...
We have pre and post data for 17 of the 20 reinforcers. All lost weight. The mixed social group started out on average lower than the exclusive group.
...
While the exclusive group lost 20.55g more this effect wasn't significant, but perhaps still biologically relevant. Both groups lost on average 119.45g.
...
Note to self: need to open tab_model and resave as xlsx.

# Statistical analysis residents

We want to know if the residents change their behaviour in response to the reinforcing release. To test this we compare their movement metrics before and after the reinforcement for the same period i.e. 58 days either side. Unlike the reinforcer dates, we are measuring post-release from the *first* reinforcer released, rather than the last - because these 3 days still have potential to impact the residents. 

```{r}
# define pre-release date period
pre_dates <- seq.Date(from = as_date("2023-04-11"), to = as_date("2023-06-07"), by = "day")

# define post-release date period
post_dates <- seq.Date(from = as_date("2023-06-07"), to = as_date("2023-08-03"), by = "day")
...
**Results:**
Residents moved less distance post reinforcement and the trend was different. 
...
# parsimonious model where delta <2 and fewest predictors 
# in this case only first model had fewest
...
Residents had been increasing their roost displacement, with a spike at reinforcement, but this declined. 

**Interpretation:**
The residents were disrupted by the reinforcers and had a short-term decrease in roost fidelity - perhaps to accommodate the move to the shared roost area. They then had good fidelity - perhaps reciprocal anchoring effect.
...
**Results:**
The main effect of period is not significant (p=0.679). Residents were already moving closer to the release site over time before reinforcement.
