...
# summary statictics of weights
hist(res_interact$weight)
mean(res_interact$weight)
sd(res_interact$weight)
```
...
res_graph <- graph_from_adjacency_matrix(res_interact, mode = "undirected", diag = FALSE, weighted = TRUE)
...
# detect communities
eb <- cluster_edge_betweenness(res_graph)
length(eb)
...
The network density is 87%. The network is continuous but has three communities. This aligns with my observation of the birds from the tracking data, where there was a group in zone 2 (centred on Nutmeg) and a group in zone 1 (centred on Marmalade) with Aurora floating between the two groups. 
