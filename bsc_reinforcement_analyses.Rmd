...
ggraph(networkfr)+
  ...
  # node basic
  geom_node_point(size = 7, shape = 20)+
  # add labels
...
Instead of using dyad connections, we can use the social groups (which are fortunately nearly evenly split among the reinforcers) to ask questions about conspecific interactions. Social groups can tell you more about the overal social environment than dyads alone (Farine and Whitehal 2015).
...
The mean distance moved (and distance moved over time) for the mixed-reinforcers was more similar to the residents than the exclusive-reinforcer group, but not a pronounced difference.
...
m1 <- lmer(daily_dist ~ social + (1|id), data = datam1)
summary(m1)
emmeans(m1, pairwise ~ social, adjust = "tukey", type = "response")
...
# test whether establishment differs between the groups
m2 <- lmer(log(distance_previous) ~ social + (1|id), data = datam2)
summary(m2)
emmeans(m2, pairwise ~ social, adjust = "tukey", type = "response")
...
The exclusive group is significantly different to both the mixed and resident groups (which were not different from one another).

The larger distances between roosts indicates the exclusive group were less settled and kept moving, while the the mixed group moved less (and the reinforcers decreased over time).
...
m3 <- lmer(sqrt(dist_release) ~ social + (1|id), data = datam3)
summary(m3)
emmeans(m3, pairwise ~ social, adjust = "tukey", type = "response")
...
The exclusive group moved significantly further from the release site than the mixed and resident groups (which were not different from one another).
