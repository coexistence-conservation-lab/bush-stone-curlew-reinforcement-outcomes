...
We removed points after the end of the study period (60 days after  at the start of next experiment - translocation to Orana).
...
```{r}
# core range polygon per bird per day 🐢🐢 2.5 hours
...
for (i in 1:length(birds)){
  for (j in 1:length(days)){
    ...
    hr50_daily <- rbind.data.frame(hr50_daily, kud_df_utm)
  }}

# extract area per day i.e. drop geometry
hr50_area <- st_drop_geometry(hr50_daily)

# save to file
write.csv(hr50_area, "results/daily_hr50_area.csv", row.names = TRUE)
```

Home range 90% KUD - changed grid and extent to optimise, takes 1 hour now. 
...
for (i in 1:length(birds)){
  for (j in 1:length(days)){
    ...
    kud <- kernelUD(points[,1], h="href", grid=400, extent = 7) %>% 
...
m7 <- glmmTMB(daily_dist ~ period * (scale(as.numeric(date))) + (1|id), 
              data = datam7,
              na.action = "na.fail")
...
**Results:**
The best fitting model included period (pre or post reinforcement), date (linear), the interaction between these, and individual as a random factor. We didn't use quadratic terms due to very inflated collinearity with period. 

Residents moved significantly less distance post reinforcement and the trend was different. 

On average, residents moved 1028m less in the post-reinforcement period (p < 0.001). The interaction of time and period was significant p < 0.001), where distance moved was stable prior to reinforcement, whereas post-reinforcement it dropped and then slowly rose to pre-reinforcement levels. 
...
# distribution of movement data
hist(datam8$distance_previous)
ggqqplot(datam8$distance_previous)

# correct left skew
hist(log1p(datam8$distance_previous))
ggqqplot(log1p(datam8$distance_previous))

# test difference pre and post reinforcement
m8 <- glmmTMB(log1p(distance_previous) ~ period * (scale(as.numeric(date))) + (1|id), 
              data = datam8,
              na.action = "na.fail")
...
# distribution of movement data
hist(datam9$dist_release)
ggqqplot(datam9$dist_release)

# correct left skew - not perfect but okay
hist(log1p(datam9$dist_release))
ggqqplot(log1p(datam9$dist_release))

# test difference pre and post reinforcement
m9 <- glmmTMB(log1p(dist_release) ~ period * (scale(as.numeric(date))) + (1|id), 
             data = datam9,
             na.action = "na.fail")
...
Residents were significantly closer to the release site post-reinforcement. The effect of time was overall a decline in distance, but post-reinforcement showed a positive trend of time that differed from pre-reinforcement.

**Interpretation:**
The residents were already moving back towards the release area, but this was enhanced by the addition of the reinforcers. 
...
```{r}
# read in data and filter to residents for reinforcement period
datam10 <- read.csv("results/daily_hr90_area.csv") %>%
 # add metadata
 left_join(dplyr::select(metadata, c("id", "cohort"))) %>%
 # select only residents
 filter(cohort == "Resident") %>%
...
# plot daily distance over time as period means
ggplot(sum10, aes(date, dist, ymin = lower, ymax = upper)) +
 geom_path() +
 geom_ribbon(alpha = 0.2) +
 xlab("Days post-reinforcement") + 
...
# distribution of movement data
hist(datam10$area)
ggqqplot(datam10$area)

# correct left skew
hist(log1p(datam10$area))
ggqqplot(log1p(datam10$area))

# test difference pre and post reinforcement
m10 <- glmmTMB(log1p(area) ~ period * (scale(as.numeric(date))) + (1|id), 
             data = datam10,
             na.action = "na.fail")
...
# parsimonious model where delta <2 and fewest predictors 
# in this case only one model (top ranked) had delta AICc <2
best10 <- get.models(dm10, delta<2)[[1]]
...
pred_data_pre <- expand.grid(
 date = seq(min(datam10$date[datam10$period == "pre-reinforcement"]),
            release_date - 1, by = 1),
 period = "pre-reinforcement"
)

pred_data_post <- expand.grid(
 date = seq(release_date, max(datam10$date[datam10$period == "post-reinforcement"]), by = 1),
 period = "post-reinforcement"
)

# Combine and add necessary covariates (date2, id, etc.)
pred10 <- rbind(pred_data_pre, pred_data_post) %>%
 mutate(id = NA)

# Predict values using the valid dataset
values10 <- predict(best10, newdata = pred10, type = "response", se.fit = TRUE) %>%
 as.data.frame() 

# Add predictions to dataframe - back transform the log values using exponential
pred10 <- pred10 %>%
 mutate(
   predicted = expm1(values10$fit),
   lower = expm1(values10$fit - 1.96 * values10$se.fit),
   upper = expm1(values10$fit + 1.96 * values10$se.fit)
 ) %>%
 mutate(date = as_date(date))

# plot predictions
ggplot(pred10, aes(date, predicted, ymin = lower, ymax = upper)) +
 geom_line()+
 geom_ribbon(alpha = 0.2)+
 xlab(element_blank()) + 
...
Home range was significantly smaller post-reinforcement. Pre-reinforcement increased with time whereas post-reinforcement was stable (slight decrease). 

**Interpretation:**
Contrary to hypothesis, they did not need bigger home range to compensate for competition. They actually stabilised their home range size. The increase April to June may have been due to seasonal food availability. 
