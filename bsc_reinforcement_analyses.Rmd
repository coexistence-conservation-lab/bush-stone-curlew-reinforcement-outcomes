...
# Packages
pacman::p_load(beepr, janitor, SDLfilter, tidyverse)
...
We needed to clean the data to remove GPS errors. We cleaned the data using a combination of: 

* quality indices (number of satellites)
* biologically realistic movement (based on altitude) and 
* biologically realistic movement (based on speed and turning angles). 

## Quality indices

We removed duplicate values and fixes taken with less than 4 satellites using the dupfilter from SDLfilter. 
...
data_dupfilt <- dupfilter(data_raw)
```
...
A test case on one bird was previously used to refine the user input threshold values until visual inspection showed good performance on reducing obvious outliers (not included here). This was applied to all birds. 
