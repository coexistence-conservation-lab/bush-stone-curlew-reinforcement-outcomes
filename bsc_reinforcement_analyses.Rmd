...
# Plot points with angles - test one bird
temp <- filter(data_speed, id == "Zeus")
ggmap(map_z15)+
...
# Plot points with speeds - test one bird
temp <- filter(data_speed, id == "Zeus")
ggmap(map_z15)+
  geom_path(data = temp, aes(location_long, location_lat), colour = "white")+
...
We defined the biologically realistic maximum speed as 16.6 m/s (60km/h) based on previous work of potential flight speed with wind assistance - also reported for other waders in flight. 

We defined the biologically realistic turning speed as 2m/s for angles greater than 90. 

```{r}
# Apply turning angle and speed filtering
data_sfilt <- data_speed %>%
  filter(speed_in < 16.6) %>%
  filter(speed_out < 16.6) %>%
  filter(!(speed_out > 2 & angle > 90)) %>%
  filter(!(speed_in > 2 & angle  > 90))
...
Percentage original data removed: 4%
...
Thin data to uniform sampling interval and reduce signal to noise ratio. 
...
The mean sampling rate for the cleaned data is 6 minutes. We will use a time parameter of 3 minutes (half the sampling interval) following advice from the wildlifeDI package vignette. 
...
The test dyad took 1:49 so I'm anticipating the full set of 78 interactions will take 2.3 hours. 
...
Mean proximity score is 13%, min is no interaction and max is 70%. 
...
Mean proximity score is 11%, min is <1% and max is 37%. 
...
We have a continuous network of 13 individuals (nodes) and 70 edges with an edge density of 89.7%.
...
Next calculate community. Based on the suggestions by [Yang et al. (2016)](https://www.nature.com/articles/srep30750). Walktrap and spinglass had same results: two communities. 
