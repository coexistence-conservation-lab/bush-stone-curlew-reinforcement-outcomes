...
# Packages
pacman::p_load(adehabitatHR, amt, asnipe, atlastools, beepr, ggfortify, ggmap, ggnewscale, ggpubr, ggridges, gtools, igraph, janitor, lme4, lmerTest, move2, momentuHMM, scattermore, sf, sp, STRAND, suncalc, survival, survminer, terra, tidyterra, tidyverse, wildlifeDI)
...
STRAND was developed for directional networks, but can be used for undirectional networks with care for inferences. 

We need an exposure matrix to account for potential for interactions to occur. For this we use the total number of fixes a dyad were simultaneously tracked for. 

We will calculate social networks for 2 time periods:
...
## Comparison residents and reinforcers 
...
m1 <- lmer(dist_sqrt ~ cohort * elapsed + (1|id), data = establishment_55)
summary(m1)
anova(m1)
```
...
# test whether establishment differs between the groups
m2 <- lmer(dist_log ~ cohort * elapsed + (1|id), data = establishment_55)
summary(m2)
anova(m2)
```
...
```{r}
# read in data
home_range <- read.csv("data/hr_daily_area.csv") %>%
   # add metadata
  left_join(dplyr::select(metadata, c("id", "cohort", "start_date"))) %>%
  # format dates as dates
  mutate(date = as_date(date),
...
# plot distance moved between roosts by cohort during establishment
ggplot(summary)+
  geom_path(aes(elapsed, mean_area, colour = cohort))+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("Days post-release") + 
  ylab("Mean home range size (hectares)")
```

Reinforcers initially have much large home ranges (due to the shuttle between the central woodland and the eastern fenceline) than the residents, before settling.
...
# distribution of movement data
hist(establishment_55$area)
ggqqplot(establishment_55$area)

# correct left skew
establishment_55 <- establishment_55 %>%
  mutate(area_log = log(area))

hist(establishment_55$area_log)
ggqqplot(establishment_55$area_log)

# plot difference between cohorts (without time)
ggplot(establishment_55)+
  geom_boxplot(aes(cohort, area_log))+
  theme_bw()+
  xlab("Cohort") + 
  ylab("Home range area (log transformed)")
...
# test whether establishment differs between the groups
m4 <- lmer(area_log ~ cohort * elapsed + (1|id), data = establishment_55)
summary(m4)
anova(m4)
```

We found a significant effects of elapsed time and the interaction of time and cohort on the size of home ranges. Residents started with small (1ha) home ranges and gradually increased over time (to 3ha) whereas reinforcers started with large home ranges (8ha) and gradually decreased (to 6ha). 
