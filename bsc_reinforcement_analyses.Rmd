...
mtr <- rbind(
  ...
  # transpose
  t() %>%
  # add a buffer for GPS accuracy of 30m for analysis, set to 0m for mapping
  buffer(30)
```
...
**Results:**
The best fitting model included period (pre or post reinforcement), date (linear and quadratic), the interaction between these, and individual as a random factor. We didn't use quadratic terms due to very inflated collinearity with period. 

Residents moved significantly less distance post reinforcement and the trend was different. 

On average, residents moved 829m less in the post-reinforcement period (p < 0.001). The interaction of time and period was significant p < 0.001), where distance moved was stable prior to reinforcement, whereas post-reinforcement it dropped and then slowly rose to pre-reinforcement levels. 
...
# test difference pre and post reinforcement
m7 <- glmmTMB(log1p(distance_previous) ~ period * (scale(as.numeric(date))) + (1|id), 
              data = datam7,
              na.action = "na.fail")
...
# test difference pre and post reinforcement
m8 <- glmmTMB(log1p(dist_release) ~ period * (scale(as.numeric(date))) + (1|id), 
             data = datam8,
             na.action = "na.fail")
...
# test difference pre and post reinforcement
m9 <- glmmTMB(log1p(area) ~ period * (scale(as.numeric(date))) + (1|id), 
             data = datam9,
             na.action = "na.fail")
